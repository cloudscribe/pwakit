...
namespace cloudscribe.PwaKit.Services
{
    public class DefaultGeneratePwaInitScript : IGeneratePwaInitScript
    {
        ...
        public Task<string> BuildPwaInitScript(HttpContext context, IUrlHelper urlHelper)
        {
            ...
            script.Append("if(window.swRegisteredHandler) {");
            script.Append("window.swRegisteredHandler(serviceWorkerRegistration);");
            script.Append("console.log(`serviceworker passed to other scripts`);");
            script.Append("}");
            ...
            //script.Append("wb.getSW().then(function(sw) {");

            
            ////passing sw to the push opt in scripts
            //script.Append("if(window.swRegisteredHandler) {");
            //script.Append("window.swRegisteredHandler(sw.ServiceWorkerGlobalScope.registration);");
            //script.Append("console.log(`serviceworker passed to other scripts`);");
            //script.Append("}");
